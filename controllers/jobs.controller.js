...
module.exports.index = async (req, res) => {
  try {
    ...
    const cleanPositions = positions.map((position) => position.dataValues);
...
module.exports.updateJobPatch = async (req, res) => {
    ...
    try {
      ...
      const existJobName = await dbHuman.Position.findOne({
        where: {
      ...
      await dbPayroll.Position.update(
        {
          PositionName: jobName,
        },
        {
      ...
      req.flash("thanhcong", "Cập nhật chức vụ thành công!");
      res.redirect("/jobs");
    } catch (error) {
      console.log(error);
      res.redirect("/jobs");
...
module.exports.deleteJob = async (req, res) => {
  ...
  try {
    ...
    if (employeeCount > 0) {
      req.flash(
        "thatbai",
        `Không thể xóa chức vụ vì đang có ${employeeCount} nhân viên thuộc chức vụ này!`
      );
      return res.redirect("/jobs");
    }
